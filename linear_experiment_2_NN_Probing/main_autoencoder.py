...
def train_probing_network(output_dir, train_layers, device, project = False):
    """
    Trains an autoencoder for each specified layer.
    ...
    class EarlyStopping:
        ...
        def __call__(self, val_loss, model, optimizer, epoch, hparams_1=None):
            score = -val_loss
    ...
    model_name_safe = hparams_1.model_name.replace('/', '_')
    ...
    for l_idx in tqdm(train_layers, desc=f"Training autoencoder per layer at {probes_dir}"):
        
        early_stopper = EarlyStopping(patience=10, save_path = os.path.join(probes_dir, f"autoencoder_layer_{l_idx}.pt"))
        ...
        # For autoencoder, we only need the activations (X), labels are ignored
        x_train_t = t.tensor(x_data, dtype=x_data.dtype)
        x_test_t  = t.tensor(x_val, dtype=x_data.dtype)
        
        train_ds = TensorDataset(x_train_t)
        val_ds  = TensorDataset(x_test_t)
        
        train_loader = DataLoader(train_ds, batch_size=hparams_1.batch_size, shuffle=True)
        val_loader = DataLoader(val_ds, batch_size=hparams_1.batch_size, shuffle=False)
        hparams_1.total_steps = len(train_loader) * hparams_1.num_epochs

        try:
            print('probe file path: ', os.path.join(probes_dir, f"autoencoder_layer_{l_idx}.pt"))
            checkpoint = t.load(os.path.join(probes_dir, f"autoencoder_layer_{l_idx}.pt"), map_location="cpu")
            model = Autoencoder().to(device)
            model.load_state_dict(checkpoint["model_state_dict"])
            optimizer = t.optim.Adam(model.parameters(), lr=hparams_1.lr)
            optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
            start_epochs = checkpoint["epoch"]
            print(f"\n--- Training autoencoder for Layer {l_idx} from epoch {start_epochs}. Existing train files found ---")
        except:
            model = Autoencoder().to(device)
            optimizer = t.optim.Adam(model.parameters(), lr=hparams_1.lr)
            criterion = t.nn.MSELoss()  # Use MSELoss for standardized data, or L1Loss
            scheduler = t.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda)
            writer = SummaryWriter(log_dir=hparams_1.logdir)
            start_epochs = 0
            print(f"\n--- Training autoencoder for Layer {l_idx} from start. No existing train files found ---")

        # Initialize criterion and scheduler outside try-except to ensure they exist
        criterion = t.nn.MSELoss()
        scheduler = t.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda)
        writer = SummaryWriter(log_dir=hparams_1.logdir)
        step = 0

        for epoch in range(start_epochs, hparams_1.num_epochs):
            config = {
                "layer": l_idx,
                "learning_rate": hparams_1.lr,
                "epochs": hparams_1.num_epochs,
                "model_type": "autoencoder",
                "batch_size": hparams_1.batch_size
            }
            run_name = f"layer_{config['layer']}_autoencoder_lr{config['learning_rate']}_batchsize{config['batch_size']}"
            run = wandb.init(
                entity='jerrycloud3316-ai-club-iit-madras',
                project='training autoencoders on triviaqa activations on gemma-2-2b-it',
                name=run_name,
            ...
            pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{hparams_1.num_epochs} [Training]", leave=False)
            
            for batch_tuple in pbar:
                try:
                    X_batch = batch_tuple[0]  # Get first element from tuple
                    progress = pbar.n / pbar.total
                    
                    # Save checkpoint at 10% intervals
                    if progress > 0 and int(progress * 10) > int((progress - 1/pbar.total) * 10):
                        t.save({
                            "model_state_dict": model.state_dict(),
                            "hparams_1": hparams_1,
                            "optimizer_state_dict": optimizer.state_dict(),
                            "epoch": epoch
                        }, os.path.join(probes_dir, f'autoencoder_layer_{l_idx}.pt'))
                        print(f"Model saved at {int(progress*100)}% progress")
                    ...
                    # Autoencoder: reconstruct the INPUT, not labels
                    loss = criterion(reconstructed, X_batch)
                    
                    loss.backward()
                    optimizer.step()
                    
                    if scheduler.get_last_lr()[0] < hparams_1.lr:
                        scheduler.step()
                    ...
                    with t.no_grad():
                        mse = t.nn.functional.mse_loss(reconstructed, X_batch)
                        # Percentage of variance explained
                        variance_explained = 1 - (mse / (X_batch.var() + 1e-8))
                    
                    run.log({
                        "loss/train": current_loss,
                        "mse/train": mse.item(),
                        "variance_explained/train": variance_explained.item(),
            ...
            est_remaining = (hparams_1.num_epochs - (epoch + 1)) * elapsed
            ...
            val_loss = 0.0
            val_mse = 0.0
            val_variance_explained = 0.0
            
            with t.no_grad():
                val_pbar = tqdm(val_loader, desc=f"Epoch {epoch+1}/{hparams_1.num_epochs} [Validation]", leave=False)
                for batch_tuple in val_pbar:
                    X_batch = batch_tuple[0]
                    X_batch = X_batch.to(t.float32).to(device)
                    
                    reconstructed = model(X_batch)
                    current_loss = criterion(reconstructed, X_batch).item()
                    ...
                    val_mse += mse.item()
                    val_variance_explained += variance_explained.item()
                    
                    val_pbar.set_postfix({
                        "val_loss": f"{current_loss:.6f}"
                    })
                    
                    run.log({
                        "loss/val_batch": current_loss,
                        "mse/val_batch": mse.item(),
                        "variance_explained/val_batch": variance_explained.item()
                    })
            
            avg_val_loss = val_loss / len(val_loader)
            avg_val_mse = val_mse / len(val_loader)
            avg_val_variance = val_variance_explained / len(val_loader)
            
            early_stopper(avg_val_loss, model, optimizer, epoch, hparams_1)
            if early_stopper.early_stop:
                print(f"Early stopping at epoch {epoch+1}")
                break
            
            print(f"Layer {l_idx} | Epoch {epoch+1} | Val Loss: {avg_val_loss:.6f}")
            print(f"Val MSE: {avg_val_mse:.6f} | Variance Explained: {avg_val_variance:.4f}")

            writer.add_scalar("Loss/val", avg_val_loss, epoch)
            writer.add_scalar("MSE/val", avg_val_mse, epoch)
            writer.add_scalar("VarianceExplained/val", avg_val_variance, epoch)
            
            run.log({
                "loss/val_epoch": avg_val_loss,
                "mse/val_epoch": avg_val_mse,
                "variance_explained/val_epoch": avg_val_variance
            })

            run.finish()

        writer.close()
        print(f"Training completed for layer {l_idx}")
#Main
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generations_file', type=str, default=None, help="Name of the JSON file with generated answers.")
    parser.add_argument('--generations_dir', type=str, default=None, help="directory where the .json files are")
    ...
    hparams_1.model_name = args.model_repo_id
