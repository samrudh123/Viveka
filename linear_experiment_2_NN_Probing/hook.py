...
#     start_index=0,
#     end_index=0,
#     generations_filename='generated_completions_20k.json',
...
def get_truth_probe_activations(
    ...
    # Get the model's end-of-sentence token for appending to prompts
    eos_token = tokenizer.eos_token if hasattr(tokenizer, 'eos_token') and tokenizer.eos_token else "<eos>"
    ...
    for local_idx, stmt in enumerate(tqdm(
        ...
        # --- MODIFICATION START ---
        # Instead of creating True/False pairs, create a single prompt ending with the EOS token.
        for answer_text, ground_truth in zip(generation_data["generated_answers"], generation_data["ground_truth_labels"]):
            # Append the answer and the model's specific end-of-sentence token.
            prompt_with_eos = f"{base_prompt} {answer_text}{eos_token}"
            appended_prompts.append(prompt_with_eos)
            # Append only the original ground truth label.
            final_labels.append(ground_truth)
        # --- MODIFICATION END ---

        batch_size = 8 # how many answers of the same statement you wanna process at once
        all_last_token_resid = [[] for _ in range(model.cfg.n_layers)]  # list per layer
        ...
        for i in range(0, len(appended_prompts), batch_size):
            ...
            del cache, tokens, layer_last_token
            torch.cuda.empty_cache()
        ...
        # --- MODIFICATION START ---
        # The number of rows is now equal to the number of answers, not twice that.
        num_rows = batch_slice
        # --- MODIFICATION END ---
    ...
    # Corrected typo from t.cuda to torch.cuda
    torch.cuda.empty_cache()
    print(f"\nActivation extraction complete for this slice. Activations saved in '{activations_dir}'.")