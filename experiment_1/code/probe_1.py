...
def main():
    ...
    model_output_file = f"/kaggle/input/triviaqa-subsampled-activations-2/gemma-2-2b-instruct-answers-triviaqa.csv"
    data = pd.read_csv(model_output_file).reset_index()
    input_output_ids = torch.load(
        f"/kaggle/input/triviaqa-subsampled-activations-2/gemma-2-2b-instruct-input_output_ids-triviaqa.pt")

    if args.test_dataset is not None:
        test_dataset = args.test_dataset
    else:
        test_dataset = args.dataset
    model_output_file_test = f"/kaggle/input/triviaqa-subsampled-activations/gemma-2-2b-instruct-answers-triviaqa.csv"
    load_test = False
    if os.path.isfile(model_output_file_test):
        data_test = pd.read_csv(model_output_file_test)
        input_output_ids_test = torch.load(
            f"/kaggle/input/triviaqa-subsampled-activations/gemma-2-2b-instruct-input_output_ids-triviaqa.pt")
        load_test = True
